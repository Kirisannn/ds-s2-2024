...
public class Test4 {
    ...
    // Test 4: Varying Responsiveness & Offline Nodes, with 5 Proposals
    public static void testBehaviours() throws InterruptedException {
        System.out.println("---Test 4: 5 Voting Proposals with Different Response Profiles for All Members---");
        ...
        executor.submit(() -> nodes.get(6).startProposal("M1")); // M7 proposes M1
        executor.submit(() -> nodes.get(7).startProposal("M2")); // M8 proposes M2
        executor.submit(() -> nodes.get(8).startProposal("M3")); // M9 proposes M3
        executor.submit(() -> nodes.get(3).startProposal("M2")); // M4 proposes M2
        executor.submit(() -> nodes.get(4).startProposal("M1")); // M5 proposes M1
        ...
        System.out.println("---Test 4: Concurrent Proposals with Different Behaviours Completed.---");
    }
